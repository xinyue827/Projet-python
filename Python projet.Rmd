...
Ce projet consiste à extraire des informations sur les offres d'emploi, notamment pour les postes d'alternance situés en Île-de-France.   Notre projet est basé sur le site d'emploi Indeed. Le but est de faciliter la recherche d'emploi, en particulier pour l'étape de postulation. Notre objectif est de collecter les emails des ressources humaines (RH) en fonction des informations obtenues, d'automatiser la rédaction d'email en fonction de la description du poste, de l'email de recrutement et de l'envoi de notre CV pour effectuer la postulation.
...
## Fonction pour extrair les liens de tous les offres d'emploi dans le domain que l'on souhaite sur le site d'emploi Indeed, et on peut définir le nombre de page que l'on souhaite y chercher les liens des emplois.

Dans notre projet, on vise à extraire les liens d'emploi dans la domaine data et analyst pour 15 pages sur Indeed.
...
## Fonction pour trouvez les emails de recrutement à partir des liens de l'offre d'emplois. Le résultat affiche à la fois l'email trouvé et le lien original d'où il trouve les emails.
...
## Fonction pour créer les chemins d'envoi des mails dans le site Resend et ajouter la pièce-jointe dans le mail.
```{python}
...
## Fonction d'automatisation de l'envoi de mail de candidature, et vérifier si il est réussie
à envoyer.
```{python}
